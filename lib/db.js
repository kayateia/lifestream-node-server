...
function dbHandleError(err, db, query, final, callback) {
	console.log(err);
	if (err.code == "SQLITE_BUSY") {
		// wait for some duration, then try again
		setTimeout(function () {
			query.func(query.string, query.bindings, callback)
		}, 1000);
	}
	else {
		if (final) {
		...
		callback(models.error(err.toString()), db);
	}
}

function dbHandleResult(err, rows, db, query, final, callback) {
	if (err) {
		dbHandleError(err, db, query, final, callback);
	}
	else if (rows) {
		if (final) {
			db.close();
		}
		callback(null, rows, db);
	}
	else {
		if (final) {
			db.close();
...
// Parameters:
//   query: query string
//   bindings (optional): data bindings for query string
//   final (optional): if true, DB should be closed after this request
//   callback: callback function from REST API
function dbAll(query, bindings, final, callback) {
	var db = new sqlite3.Database("lifestream.db");
	...
	db.all(query, bindings, function(err, rows) {
		// Along with the error and result, also pass information that would
		// be needed by the error handler to re-execute the query if needed:
		//
		//   - A reference to this function
		//   - The query string passed to this function
		//   - the data bindings passed to this function
		//   - the DB object
		//
		// And the information that would be needed by the result handler to
		// respond to the REST API call:
		//
		//   - The callback passed in from the REST API
		dbHandleResult(err, rows, db, {
			func: dbAll,
			string: query,
			bindings: bindings
		}, final, callback);
...
//   final (optional): if true, DB should be closed after this request
//   callback: callback function from REST API
function dbGet(query, bindings, final, callback) {
	var db = new sqlite3.Database("lifestream.db");
	...
	db.get(query, bindings, function(err, row) {
		// Along with the error and result, also pass information that would
		// be needed by the error handler to re-execute the query if needed:
		//
		//   - A reference to this function
...
//   final (optional): if true, DB should be closed after this request
//   callback: callback function from REST API
function dbRun(query, bindings, final, callback) {
	var db = new sqlite3.Database("lifestream.db");
...
function streamList(userid, includeHidden, callback) {
	dbAll("select idd as streamid,userid,name,permission from stream"
		+ " where 1=1"
...
var userCreate = watt(function *(login, passwordHash, name, email, isadmin, next) {
	var [, row, db] = yield dbGet("select * from user where login=?", [ login ], false, next.args);

	if (row) {
		db.close();
		throw models.error("User already exists");
	}

	yield dbRun("insert into user(login, pwhash, name, email, isadmin) values (?,?,?,?,?)", [ login, passwordHash, name, email, isadmin ], next);
});

/*function userCreate(login, passwordHash, name, email, isadmin, callback) {
	dbGet("select * from user where login=?", [ login ], false, function(err, row, db) {
		if (row) {
			db.close();
			return callback(models.error("User already exists"));
		}

		dbRun("insert into user(login, pwhash, name, email, isadmin) values (?,?,?,?,?)", [ login, passwordHash, name, email, isadmin ], callback);
	});
}*/
