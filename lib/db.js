...
function streamList(userid, includeHidden, callback) {
	dbExec(function(db) {
		db.all("select stream.id as id, stream.userid as userid, stream.name as name, stream.permission as permission, user.login as userLogin, user.name as userName"
			+ " from stream"
...
function streamInfo(streamid, callback) {
	dbExec(function(db) {
		db.get("select stream.id as id, stream.userid as userid, stream.name as name, stream.permission as permission, user.login as userLogin, user.name as userName"
			+ " from stream"
			+ " inner join user on stream.userid=user.id"
			+ " where stream.id=?",
			[ streamid ],
