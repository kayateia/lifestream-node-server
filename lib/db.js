	...
	dbExec(function(db) {
		...
		db.all("select id, login, name"
			...
			function(err, rows) {
				db.close();
				if (err) {
					return callback(models.error("Database error searching users", err));
				}
...
function streamList(userid, includeHidden, callback) {
	dbExec(function(db) {
		db.all("select stream.id as streamid, stream.userid as userid, stream.name as name, stream.permission as permission, user.login as userLogin, user.name as userName"
			+ " from stream"
...
function streamContents(streamid, maxnum, olderthan, callback) {
	dbExec(function(db) {
		...
		if (olderthan) {
			queryString = "select image.id as id, image.fn as fn, user.login as userLogin, user.name as userName, image.uploadtime as uploadtime, image.comment as comment, count(streamimage.streamid) as streamcount"
				+ " from image"
				...
				+ " limit ?";
			args = [ olderthan, maxnum ];
		}
		else {
			console.log("checkpoint 2");
			queryString = "select image.id as id, image.fn as fn, user.login as userLogin, image.uploadtime as uploadtime, image.comment as comment, count(streamimage.streamid) as streamcount"
				+ " from image"
				+ " inner join user on image.userid=user.id"
				+ " inner join streamimage on image.id=streamimage.imageid"
				+ " where streamimage.streamid in(" + streamid + ")"
		...
		db.all(queryString, args, function(err, rows) {
				if (err) {
					db.close();
					return callback(models.error("Database error listing stream contents", err));
				}
...
function streamInfo(streamid, callback) {
	dbExec(function(db) {
		db.get("select stream.id as streamid, stream.userid as userid, stream.name as name, stream.permission as permission, user.login as userLogin, user.name as userName"
			+ " from stream"
			+ " inner join user on stream.userid=user.id"
			+ " where stream.id=?",
			[ streamid ],
...
function streamSearch(terms, callback) {
	dbExec(function(db) {
		...
		db.all("select stream.id as id, user.id as userid, user.login as userLogin, user.name as userName, stream.name as name, stream.permission as permission"
			+ " from stream"
			+ " inner join user on stream.userid=user.id"
			+ " where stream.permission in(" + PERM_PUBLIC + "," + PERM_APPROVAL + ") and ("
			+ conditions
			+ ")",
			subterms,
			function(err, rows) {
				db.close();
				if (err) {
					return callback(models.error("Database error searching streams", err));
				}
...
function imageGetStreams(imageid, callback) {
	dbExec(function(db) {
		...
		db.all("select stream.id as id, stream.name as name, stream.permission as permission, stream.userid as userid, user.login as userLogin, user.name as userName"
			+ " from streamimage"
			...
			function(err, rows) {
				db.close();
				if (err) {
					return callback(models.error("Database error getting list of streams containing image", err));
				}
...
function imageAddStream(imageid, streamid, callback) {
	dbExec(function(db) {
		db.get("select imageid, streamid"
			+ " from streamimage"
			...
			function(err, row) {
				if (err) {
					db.close();
					return callback(models.error("Database error checking stream-image association", err));
				}
				else if (row) {
					db.close();
					return callback(models.error("Image is already associated with stream"));
				}

				console.log("checkpoint 1: " + imageid + ", " + streamid);
				db.run("insert into streamimage(imageid, streamid)"
					+ " values(?, ?)",
					[ imageid, streamid ],
					function(err) {
						if (err) {
							db.close();
							return callback(models.error("Database error associating image with stream", err));
						}
...
function imageRemoveStream(imageid, streamid, callback) {
	dbExec(function(db) {
		db.run("delete from streamimage where imageid=? and streamid=?",
			[ imageid, streamid ],
			function(err) {
				db.close();
				if (err) {
					callback(models.error("Database error dissociating image from stream", err));
				}
				else {
					callback();
				}
	...
	dbExec(function(db) {
		db.get("select image.id"
			...
			function(err, row) {
				if (err) {
					db.close();
					callback(models.error("Database error checking for permission to view image", err));
				}
				else if (!row) {
					db.all("select streamimage.streamid"
						+ " from streamimage"
						+ " inner join stream on streamimage.streamid=stream.id"
						+ " where streamimage.imageid=?"
						...
						function(err, rows) {
							if (err) {
								...
								db.all("select subscription.streamid"
									+ " from subscription"
									+ " inner join streamimage on subscription.streamid=streamimage.streamid"
									+ " where streamimage.imageid=?"
									+ " and subscription.userid=?"
									+ " limit 1",
									[ imageid, userid ], function(err, rows) {
										db.close();
										if (err) {
											callback(models.error("Database error checking for permission to view image", err));
										}
										else if (!rows) {
											callback(models.error("Permission denied"));
										}
										else  {
											// User can view image if it is in
											// at least one stream to which
											// they subscribe
											callback();
										}
									}
								...
								db.close();
								// Anyone can view an image contained in at
								// least one public stream
								callback();
							}
						}
					);
	...
	dbExec(function(db) {
		...
		db.get("select image.id"
			...
			function(err, row) {
				db.close();
				if (err) {
					callback(models.error("Database error checking for permission to edit image", err));
				}
				else if (!row) {
					callback(models.error("Permission denied"));
				}
...
function inviteList(streamid, callback) {
	dbExec(function(db) {
		db.all("select invitation.streamid as streamid, invitation.userid as userid, user.login as userLogin, user.name as userName"
			+ " from invitation"
	...
	dbExec(function(db) {
		db.get("select streamid, userid from invitation where streamid=? and userid=?",
			...
			function(err, row) {
				if (err) {
					db.close();
					return callback(models.error("Database error checking invite", err));
				}
				else if (row) {
	...
	dbExec(function(db) {
		db.get("select streamid, userid"
			...
			function(err, row) {
				db.close();
				if (err) {
					callback(models.error("Database error checking invite", err));
				} else if (!row) {
					callback(models.error("No invite found"));
				} else {
					callback(null, row);
				}
			}
...
function subscriptionListByUserId(userid, callback) {
	dbExec(function(db) {
		db.all("select subscription.streamid as streamid, stream.name as streamName, user.id as userid, user.login as userLogin, user.name as userName"
			+ " from subscription"
			...
			function(err, rows) {
				db.close();
				if (err) {
					callback(models.error("Database error listing subscriptions", err));
				} else {
					callback(null, rows);
				}
			}
...
function subscriptionListByStreamId(streamid, callback) {
	dbExec(function(db) {
		db.all("select subscription.streamid as streamid, subscription.userid as userid, user.login as userLogin, user.name as userName"
		 	+ " from subscription"
...
function subscriptionCreate(streamid, userid, callback) {
	dbExec(function(db) {
		db.get("select streamid, userid from subscription where streamid=? and userid=?",
			[ streamid, userid ],
			function(err, row) {
				if (err) {
					db.close();
					return callback(models.error("Database error checking subscription", err));
				}
				else if (row) {
				...
				db.get("select permission"
					...
					function(err, row) {
						if (err) {
							db.close();
							return callback(models.error("Database error checking stream permission", err));
						}
						else if (!row) {
							db.close();
							return callback(models.error("Stream not found"));
						}
						else if (row.permission != PERM_PUBLIC) {
							...
							db.get("select streamid, userid"
								...
								function(err, row) {
									if (err) {
										...
										db.run("delete from invitation"
											+ " where streamid=? and userid=?",
											[ streamid, userid ],
											function(err) {
												if (err) {
													db.close();
													callback(models.error("Database error consuming invite", err));
												}
												else {
													// Allow subscription
													db.run("insert into subscription(streamid, userid) values(?, ?)",
														[ streamid, userid ],
														function(err) {
															db.close();
															if (err) {
																callback(models.error("Database error creating subscription", err));
															}
															else {
																callback();
															}
...
function subscriptionDelete(streamid, userid, callback) {
	dbExec(function(db) {
		db.run("delete from subscription where streamid=? and userid=?",
			[ streamid, userid ],
			function(err) {
				db.close();
				if (err) {
					callback(models.error("Database error deleting subscription", err));
				}
				else {
					callback();
				}
	...
	dbExec(function(db) {
		db.get("select streamid, userid"
			...
			function(err, row) {
				db.close();
				if (err) {
					callback(models.error("Database error checking subscription", err));
				} else if (!row) {
					callback(models.error("No subscription found"));
				} else {
					callback(null, row);
				}
			}
