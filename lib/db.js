...
function plainSelect(statement, args, errorMessage, callback) {
	...
	sal.connect().then(function(conn) {
		...
		callback(models.error(errorMessage, err));
	}).finally(function() {
		sal.close(db);
	});
}
...
function userDelete(login, callback) {
	...
	sal.connect().then(function(conn) {
		...
		if (!rows.length) {
			callback(models.error("User " + login + " doesn't exist"));
			throw exit;
		}
...
function streamDelete(streamid, callback) {
	...
	sal.connect().then(function(conn) {
		...
		callback(models.error("Error deleting stream", err));
	}).finally(function() {
		sal.close(db);
	});
}
...
function imageAdd(userid, streamids, filename, comment, callback) {
	...
	sal.connect().then(function(conn) {
		...
		sal.run(db, "rollback").then(function() {
			...
			callback(models.error("Error rolling back image add", err));
		});
	}).finally(function() {
		sal.close(db);
	});
...
function deviceGetForStreams(streams, callback) {
	...
	sal.connect().then(function(conn) {
		...
		callback(models.error("Error finding devices for user", err));
	}).finally(function() {
		sal.close(db);
	});
}
...
function subscriptionCreate(streamid, userid, callback) {
	...
	sal.connect().then(function(conn) {
		...
		if (rows.length) {
			...
			callback(models.error("User is already subscribed to stream"));
			throw exit;
		}
