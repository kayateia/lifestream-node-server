...
var userDelete = watt(function *(userid, callback) {
	...
	try {
		...
		// Remove all image files uploaded by the user
		rimraf(process.cwd() + "/uploads/" + userid, {}, function(err) {
			if (err) {
				return conclude(models.error("User deleted, but could not delete image files", err));
			}
		});
...
function streamContents(streamid, includeOrphans, maxNum, fromTime, fromId, direction, userid, callback) {
	let streamIdClause, streamImageJoinClause;
	let comparator = direction < 0 ? "<" : ">";
	let sortOrder = direction < 0 ? "DESC" : "ASC";

	// If the request included images not currently associated with any
	// streams, use a left outer join instead of an inner join, and include
	// null stream IDs in the result set
	if (includeOrphans) {
		streamImageJoinClause = "left outer join streamimage on image.id=streamimage.imageid";
		if (streamid.length) {
			streamIdClause = "(streamimage.streamid in(" + streamid + ") or streamimage.streamid is null) and user.id=" + userid;
		}
		else if (!streamid.length) {
			streamIdClause = "streamimage.streamid is null and user.id=" + userid;
		}
	}
	else {
		streamImageJoinClause = "inner join streamimage on image.id=streamimage.imageid";
		streamIdClause = "streamimage.streamid in(" + streamid + ")";
	}
	...
	let queryString = "select image.id as id, image.fn as fn, user.id as userid, user.login as userLogin, user.name as userName, image.uploadtime as uploadtime, image.comment as comment, count(streamimage.streamid) as streamcount"
		...
		+ " " + streamImageJoinClause
		+ " where " + streamIdClause
		+ (fromTime ? " and image.uploadtime " + comparator + " ?" : "")
		+ (fromId ? " and image.id " + comparator + " ?" : "")
		+ " group by image.id"
		+ " order by image.uploadtime " + sortOrder + ", image.id " + sortOrder
		+ " limit ?";
	let args = [];
	if (fromTime)
		args.push(fromTime);
...
// Callback should take a row and an error argument. In case of error, a models.error() will be passed.
var imageDelete = watt(function *(imageid, userid, callback) {
	var db, rows;
	...
	try {
		...
		// Confirm that the specified image is owned by the requesting user
		rows = yield sal.select(db,
			"select userid, fn from image where id=? and userid=?",
			[ imageid, userid ]);
		if (!rows.length) {
		...
		yield sal.transact(db,
			// Dissoaciate the image from all streams
			"delete from streamimage where imageid=?;"
			// Delete the image from the database
			+ " delete from image where id=?;",
			[ imageid, imageid ]);

		// Delete the image file from disk
		rimraf(process.cwd() + "/uploads/" + rows[0].userid + "/" + rows[0].fn, {}, function(err) {
			if (err) {
				return conclude(models.error("Image was deleted from database, but could not be deleted from disk", err));
			}
		});
		...
		// Acquiring connection failed
		return conclude(models.error("Error deleting image from database", err));
	}
