...
function streamDelete(streamid, callback) {
	dbExec(function(db) {
		db.serialize(function() {
			...
			db.run("delete from invitation where streamid=?", [ streamid ], handleError);

			// Unsubscribe all users from stream.
			db.run("delete from subscription where streamid=?", [ streamid ], handleError);

			// Disassociate all images from this stream.
			db.run("delete from streamimage where streamid=?", [ streamid ], handleError);

			// Delete the stream.
			db.run("delete from stream where id=?",	[ streamid ], handleError);
	...
	dbExec(function(db) {
		db.all("select invitation.streamid as streamid, invitation.userid as userid, user.login as userLogin from invitation"
			...
			function(err, rows) {
				db.close();
				if (err) {
					callback(models.error("Error listing invites", err));
				} else {
					callback(null, rows);
				}
			}
