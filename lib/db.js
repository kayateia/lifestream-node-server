...
function imageGetStreams(imageid, callback) {
	dbExec(function(db) {
		...
		db.all("select stream.id as id, stream.name as name, stream.permission as permission, stream.userid as userid, user.login as userLogin"
			+ " from streamimage"
...
function subscriptionListByUserId(userid, callback) {
	dbExec(function(db) {
		db.all("select subscription.streamid as streamid, stream.name as streamName, user.id as ownerid, user.login as ownerLogin"
			+ " from subscription"
...
function subscriptionListByStreamId(streamid, callback) {
	dbExec(function(db) {
		db.all("select subscription.streamid as streamid, subscription.userid as userid, user.login as userLogin"
		 	+ " from subscription"
