		...
		if (!row) {
			...
			db.serialize(function() {
				...
				db.run("create table streamimage ("
					+ "imageid int,"
					+ "streamid int,"
					+ "foreign key(imageid) references image(rowid),"
					+ "foreign key(streamid) references stream(rowid))");
				...
				db.run("create table invitation ("
					...
					+ "userid int,"
					+ "foreign key(streamid) references stream(rowid),"
					+ "foreign key(userid) references user(rowid))", function() { innards(db); });
			});
		} else {
...
function userGetById(id, callback) {
	dbExec(function(db) {
		db.get("select rowid,login,pwhash,name,email,enabled,isadmin from user where rowid=?", [ id ], function(err, row) {
			if (err) {
				callback(err);
			}
...
function userGetByLogin(login, callback) {
	dbExec(function(db) {
		db.get("select rowid,login,pwhash,name,email,enabled,isadmin from user where login=?", [ login ], function(err, row) {
			if (err) {
				callback(err);
			}
			else if (row == undefined) {
...
function streamList(userid, callback) {
	dbExec(function(db) {
		db.all("select rowid as streamid,userid,name,permission from stream", function(err, rows) {
			db.close();
			if (err) {
				callback(err);
			} else {
...
function streamContents(streamid, maxnum, callback) {
	dbExec(function(db) {
		db.all("select image.rowid as id,image.fn as fn,user.login as userLogin,image.uploadtime as uploadtime,"
...
function imageGet(imageid, callback) {
	dbExec(function(db) {
		db.all("select userid, fn, comment, uploadtime from image where rowid=?",
			[ imageid ],
			function(err, rows) {
				if (err)
					callback(err, null);
