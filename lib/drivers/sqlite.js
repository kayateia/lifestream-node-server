...
function createDatabase(db, callback) {
	...
	db.serialize(function() {
		...
		db.run("create table stream ("
			...
			+ "permission int,"
			+ "foreign key(userid) references user(id))");
		db.run("insert into stream(userid, name, permission) values (1, 'Global Stream', " + dbconst.perms.public + ")");
		...
		db.run("create table subscription ("
			...
			+ "foreign key(userid) references user(id),"
			+ "foreign key(streamid) references stream(id))");
		db.run("insert into subscription(userid, streamid, state) values (1, 1, " + dbconst.sub.active + ")", function() { callback(null); });
	});
}
