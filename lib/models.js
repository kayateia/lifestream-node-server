...
function image(id, filename, userid, userLogin, userName, uploadTime, comment) {
	if (!id || !filename || !userLogin || !uploadTime)
		throw "Missing value in image()";
	if (!comment)
		comment = "";
	if (typeof id !== "number" || typeof filename !== "string"
		|| typeof userid !== "number" || typeof userLogin !== "string"
		|| typeof uploadTime !== "number" || typeof comment !== "string")
...
function filenameList(list) {
	if (typeof list !== "object" || (!list instanceof Array)) {
		logTypeError("list", list);
		throw "Invalid type in filenameList()";
	}
...
// Each row should be a user object.
